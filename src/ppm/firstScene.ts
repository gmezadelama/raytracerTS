...
export const exportScene = (): string => {
  let w: World = new World();
  // the scene is constructed from six spheres

  // #1 The floor is an extremely flattened sphere with a matte texture
  let floor: Sphere = new Sphere();
  ...
  // #2 The wall on the left hast the same scale and color as the floor,
  //    but is also rotated and translated into place
  let leftWall: Sphere = new Sphere();
  ...
  let rightWall: Sphere = new Sphere();
